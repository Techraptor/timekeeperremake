...
public class Calender extends JFrame {
	...
	public final ArrayList<JButton> d = new ArrayList<>(0);
	public final HashMap<JButton, ActionListener> da = new HashMap<>();
	//public ArrayList<JLabel> l = new ArrayList<>(0);
	public final ArrayList<Day> day = new ArrayList<>(0);
	public final ArrayList<PreloadedHoliday> prlddays = new ArrayList<>(0);
	public final ArrayList<JMenu> m = new ArrayList<>(0);
	...
	private void loadFederalHolidays () {
		ArrayList<Holidays> h = References.settings.usHolidays;
		Logger.logDEBUG("h.size: " + h.size());
		for (Holidays aH : h) {
		  /*
		  LocalDate date = LocalDate.now();
            date = date.withMonth(h.get(i).getMonth());
            date = date.withDayOfMonth(h.get(i).getDay());
            System.out.println(date.get(WeekFields.ISO.weekOfMonth()));
            */
			//we dont know the date of the holiday
			//we DO know the week and the DAY
			//set the week to the week we know
			//set the day to the day of that week
			//boom we have our date
			prlddays.add(new PreloadedHoliday(aH.getName(), aH.getDay(), aH.getMonth()));
		}
		for (int i = 0; i < prlddays.size(); i++) {
			//Logger.logDEBUG(new String(prlddays.get(i).getName()));
		}
	...
	private void setCalender () {
		...
		for (int i = firstDayOfMonth; i <= lengthOfMonth + firstDayOfMonth; i++) {
			if (i - firstDayOfMonth + 1 == selectedDate.getDayOfMonth() && workingDate.getMonthValue() == selectedDate.getMonthValue() && workingDate.getYear() == selectedDate.getYear()) {
				...
				day.add(new Day(workingDate.getMonthValue(), i - firstDayOfMonth + 1, +workingDate.getYear(), prlddays));
	...
	private ActionListener clicked (final int date, final int month, final int year) {
		try {
			return e -> {
				...
				day.get(date - 1).setVisible(true);
			};
		} catch (Exception e) {
			return e1 -> {
				//int m = date;
				Logger.logERROR(log + "clicked]" + e1);
				//day.get(m).setVisible(true);
			};
		}
	}
	...
	private JPanel fontGUI () {
		...
		JComboBox<Integer> size = new JComboBox<Integer>(sizes);
		...
		go.addActionListener(e -> {
			if (style.getSelectedIndex() == 0) {
				setFont(new Font(font.getItemAt(font.getSelectedIndex()), Font.PLAIN, (int) size.getSelectedItem()));
			} else if (style.getSelectedIndex() == 1) {
				setFont(new Font(font.getItemAt(font.getSelectedIndex()), Font.ITALIC, (int) size.getSelectedItem()));
			} else if (style.getSelectedIndex() == 2) {
				setFont(new Font(font.getItemAt(font.getSelectedIndex()), Font.BOLD, (int) size.getSelectedItem()));
			}
			setDateLabelFont();
			setDates();
			setCalender();
